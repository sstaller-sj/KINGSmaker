...
# Set threading limits to prevent deadlocks
os.environ.update({
...
# --- UPDATED: Trust the Universal ID from the upstream script ---
# We no longer generate seq_0000x here. We use the Seq_ID already in the file.
if 'Seq_ID' not in df.columns:
    # This is an emergency check. If it fails, something is wrong with the upstream script.
    raise KeyError(f"Seq_ID column not found in {input_path}. Check analyze_selex_enrichment.py")

# Extract a clean name for the plot title
base_name = os.path.basename(input_path).replace(".csv", "").replace("top_100_", "")

# 2. Handle Edge Case: Not enough data for UMAP
if len(df) < 5:
    df['Cluster_ID'] = -1
    df['UMAP_1'] = 0
    df['UMAP_2'] = 0
    ...
    # =========================================================================
    # IDENTIFY KINGS (Using Universal Seq_ID)
    # =========================================================================
    kings = []
    ...
    for cid in df['Cluster_ID'].unique():
        ...
        # 1. Metric Winner: First appearance in this Top 100 list
        metric_king = cluster_subset.iloc[0]
        
        # 2. Enrichment Winner: Highest value in enrichment column
        enrich_king = cluster_subset.sort_values(by=enrich_col, ascending=False).iloc[0]
        
        # 3. Centroid King: Closest to UMAP geometric center
        ...
        # 4. Consensus King: Minimum average Levenshtein distance
        ...
        for k_type, k_data in [('Metric_Winner', metric_king), 
            ...
            kings.append({
                'Cluster_ID': cid,
                'King_Type': k_type,
                'Seq_ID': k_data['Seq_ID'], # Matches IDs in all other files
